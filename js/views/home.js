...
export async function renderHome(_params) {
  ...
  const [milestoneGroups, versionGroups, dateGroups, plannedGroups] = await Promise.all([
  ...
  const versions = paired.map(x => x.versions);
      ...
      <aside class="home-sidebar">
        ...
        <div class="sidebar-project-list">
          ${projects.length === 0
            ...
            : projects.map((p, i) => renderSidebarItem(p, milestones[i], versions[i])).join('')}
        </div>
      </aside>
...
function renderSidebarItem(project, milestones, versions) {
  ...
  const completeM = milestones.filter(m => m.isComplete).length;
  const percent = totalM === 0 ? 0 : Math.round((completeM / totalM) * 100);
  ...
  return `
    <div class="sidebar-project-item${project.status === 'active' ? ' is-active' : ''}"
      ...
      <div class="milestone-progress">
        ...
        <span class="milestone-progress-label">${completeV}/${totalV} V · ${completeM}/${totalM} M</span>
      </div>
    </div>
  `;
}
